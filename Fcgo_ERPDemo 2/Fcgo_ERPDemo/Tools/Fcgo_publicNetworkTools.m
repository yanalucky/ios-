...
+ (void)reloadUserAddressDict
{
    ...
    [Fcgo_UserTools shared].userAddressDict = mutableDict;
}
...
+ (void)postCheckToken
{
    [Fcgo_NetworkManager postRequest:NSFormatHeardMeThodUrl(@"", @"checkToken") parametersContentCommon:nil successBlock:^(BOOL success, id responseObject, NSString *msg) {
        if (success) {}else{}
    } failureBlock:^(NSString *description) {}];
}
